...
def backtrack(x: int, y: int, n: int, T: List[List[int]], visited: List[List[bool]], 
              path: List[Tuple[int, int]], weight: int, step: int, min_path: List[Tuple[int, int]], 
              min_weight: List[int]) -> None:
    if len(path) == n * n:
